...
type WebcamPixelGridProps = {
  ...
  className?: string;
  /** Whether to show the built-in camera permission UI */
  showErrorUI?: boolean;
  ...
  return (
    <div className={cn("relative h-full w-full", className)}>
      ...
      <div
        aria-hidden="true"
        className={cn(
          "absolute inset-0 overflow-hidden bg-[#070707] transition-opacity duration-700",
          isReady ? "opacity-0" : "opacity-100",
        )}
      >
        <div className="webcam-pixel-fallback absolute inset-0" />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_32%_26%,rgba(37,99,235,0.32),transparent_28%),radial-gradient(circle_at_68%_64%,rgba(244,63,94,0.22),transparent_30%),radial-gradient(circle_at_42%_78%,rgba(16,185,129,0.18),transparent_26%)]" />
      </div>
      ...
      {showErrorUI && error && showErrorPopup && (
        <div className="animate-in fade-in slide-in-from-top-2 fixed top-4 end-4 z-50 duration-300">
          <div className="relative flex max-w-sm items-start gap-3 rounded-lg border border-white/10 bg-black/80 p-4 shadow-2xl backdrop-blur-xl">
            ...
            <button
              ...
              className="absolute top-2 end-2 rounded-md p-1 text-white/40 transition-colors hover:bg-white/10 hover:text-white/70"
            >
              <svg
                className="h-4 w-4"
                fill="none"
      ...
      {showErrorUI && error && !showErrorPopup && (
        <button
          ...
          className="fixed top-4 end-4 z-50 flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-black/60 text-white/50 shadow-lg backdrop-blur-xl transition-all hover:scale-105 hover:bg-black/80 hover:text-white/80"
